/**
 * JaferSQL v1.6.0 – Client‑side SQLite engine with cache integration & full persistence management
 * MIT License – Copyright (c) 2026 Jafer
 * 
 * Changelog:
 * - 1.6.0: Added clearFromOPFS() and clearFromIDB() for storage cleanup.
 * - 1.5.0: Added saveToIDB / loadFromIDB for IndexedDB persistence.
 ...
 * 
 * Enhanced: Better error messages for jaferGet/jaferAll (table/view detection).
 */
(function(global) {
  'use strict';
  ...
  JaferDatabase.prototype = {
    ...
    jaferExec: function(sql) {
      ...
      try {
        ...
        let errorMsg = `SQLite exec error: ${e.message || e}`;
        errorMsg += ` | SQL: ${sql.substring(0, 500)}`;
        throw new JaferQueryError(errorMsg, e, sql);
      }
    },
    ...
    jaferGet: function(sql, params) {
      ...
      try {
        ...
        let errorMsg = `SQLite prepare error: ${prepareErr.message || prepareErr}`;
        errorMsg += ` | SQL: ${sql.substring(0, 500)}`;
        if (params && params.length > 0) {
          errorMsg += ` | Params: ${JSON.stringify(params)}`;
        }
        const match = sql.match(/FROM\s+(\w+)/i);
        if (match) {
          errorMsg += ` | Check that table/view "${match[1]}" exists.`;
        }
        throw new JaferQueryError(errorMsg, prepareErr, sql, params);
      } finally {
        if (stmt) {
  ...
  var JaferSQL = {
    ...
    clearFromOPFS: async function(filename) {
      if (!isBrowser) throw new Error('OPFS only in browser');
      const root = await navigator.storage.getDirectory();
      try {
        await root.removeEntry(filename);
        emitProgress('opfs_clear', { filename });
      } catch (e) {
    ...
    saveToIDB: async function(db, dbName, key = 'db') {
      if (!isBrowser) throw new Error('IndexedDB only in browser');
    ...
    loadFromIDB: async function(dbName, key = 'db') {
      if (!isBrowser) throw new Error('IndexedDB only in browser');
      return new Promise((resolve, reject) => {
        const request = indexedDB.open(dbName, 1);
        request.onupgradeneeded = (e) => {
    ...
    clearFromIDB: async function(dbName, key = 'db') {
      if (!isBrowser) throw new Error('IndexedDB only in browser');
      return new Promise((resolve, reject) => {
        const request = indexedDB.open(dbName, 1);
        request.onupgradeneeded = (e) => {
