// app-ui.js – DOM manipulation, event bindings, rendering (no native dialogs)
...
if (window.innerWidth <= 768) {
  ...
  function renderDatabaseOverview() {
    ...
    try {
      encoding = logic.pragma('encoding') || 'unknown';
      pageSize = logic.pragma('page_size') || '?';
      pageCount = logic.pragma('page_count') || '?';
      freelistCount = logic.pragma('freelist_count') || '?';
      userVersion = logic.pragma('user_version') || '?';
      schemaVersion = logic.pragma('schema_version') || '?';
      journalMode = logic.pragma('journal_mode') || '?';
      autoVacuum = logic.pragma('auto_vacuum') || '?';
      synchronous = logic.pragma('synchronous') || '?';
    } catch (e) {}
  ...
  dom.tabJsonBtn.addEventListener('click', () => showJsonModal());
  ...
  // 1) Try the normal active class first
  ...
  // 2) Fallback: read from the SQL editor (set when a table is clicked)
  const editorVal = dom.sqlEditor.value.trim();
  const match = editorVal.match(/^SELECT \* FROM "([^"]+)"/i);
...
function showJsonModal() {
  ...
  // If output element is missing, stop
  if (!output) return;
...
// Close button and backdrop (unchanged)
...
document.getElementById('jsonViewerModal').addEventListener('click', (e) => {
  if (e.target === document.getElementById('jsonViewerModal')) {
    document.getElementById('jsonViewerModal').classList.add('hidden');
  }
});
...
document.getElementById('copyJsonBtn').addEventListener('click', () => {
  ...
  inspectorModal.addEventListener('click', (e) => {
    ...
    try {
      const result = logic.pragma(pragma);
  ...
  document.getElementById('inspectorTestTriggerBtn').addEventListener('click', async () => {
    ...
    try {
      ...
      const existing = logic.getObjectList('trigger').filter(t => t.name === 'audit_album_update');
      if (existing.length) {
        inspectorOutput.innerHTML = '<div class="text-red-500">Trigger already exists.</div>';
        return;
      }
      logic.createTrigger('audit_album_update', 'Album', 'AFTER', 'UPDATE', `INSERT INTO sqlite_master (type, name, tbl_name, sql) VALUES ('temp','log','Album','Updated');`);
